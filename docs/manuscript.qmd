...
n_ids_gpt <- length(unique(fcoalesce(gpt_annot$hpo_name,gpt_annot$hpo_id)))
```
...
f1a <- ggplot() + 
  ...
  ggtext::geom_textbox(aes(x=.5,y=1),
...
query_hits_dt <- data.table(
  ...
  "HPO queries"=trimws(gsub("^","",sapply(queries, paste, collapse="; <br>"), fixed = TRUE)),
...
names(code_dict) <- paste0(stringr::str_to_sentence(names(code_dict)),
                           " (",code_dict,")")
tiers_dict <- eval(args$tiers_dict)|>unlist()|>sort(decreasing = TRUE)
names(tiers_dict) <- paste0(stringr::str_to_sentence(gsub("_"," ",names(tiers_dict))),
                           " (",tiers_dict,")")
## Create the outer product of the two weight vectors
tbl_metric_weights <- as.data.table(outer(tiers_dict,code_dict, "*"),
